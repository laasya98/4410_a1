...
static void spawn(command_t command, int background){
	...
	if (background != 0 ) {
		...
		if (pid == 0){
			...
			printf("Child process running in background: %d \n", pid);
		}
	}
		...
		if (pid == 0) {
			...
			// wait on child status
			pid_t wpid = wait(&child_status);

			// wait for child processes to execute
			while(wpid != pid){
				if (WIFSIGNALED(child_status)) {
					// parent checks for each child’s exit
			...
			if (WIFSIGNALED(child_status)) {
				//return a message saying you exited due an interrupt like "ctrl+c"
				printf("Process %d terminated with signal %d \n", wpid, WTERMSIG(child_status));

			}
			else if (WIFEXITED(child_status)!=0){
				//do nothing if exited normally, otherwise print the status condition
